...
pub unsafe extern "C" fn main() -> ! {
    ...
    if abi_data == ABI_VALUE_U64 {
        let value = read_result_comap_u64(".namespace.keyname").unwrap();
        push_costack_u64(value);
    }
    else if abi_data == ABI_VALUE_U32 {
        let value = read_result_comap_u32(".namespace.keyname").unwrap();
        push_costack_u32(value);
    }
    else if abi_data == ABI_VALUE_U16 {
        let value = read_result_comap_u16(".namespace.keyname").unwrap();
        push_costack_u16(value);
    }
    else if abi_data == ABI_VALUE_U8 {
        let value = read_result_comap_u8(".namespace.keyname").unwrap();
        push_costack_u8(value);
    }
    else if abi_data == ABI_VALUE_I64 {
        let value = read_result_comap_i64(".namespace.keyname").unwrap();
        push_costack_i64(value);
    }
    else if abi_data == ABI_VALUE_I32 {
        let value = read_result_comap_i32(".namespace.keyname").unwrap();
        push_costack_i32(value);
    }
    else if abi_data == ABI_VALUE_I16 {
        let value = read_result_comap_i16(".namespace.keyname").unwrap();
        push_costack_i16(value);
    }
    else if abi_data == ABI_VALUE_I8 {
        let value = read_result_comap_i8(".namespace.keyname").unwrap();
