...
pub unsafe extern "C" fn main() -> ! {
    ...
    if abi_data == ABI_VALUE_U8 + ABI_ARRAY_BIT {
        ...
        read_result_comap_fixed_array_u8(".namespace.keyname", &mut value).unwrap();
        push_costack_array_u8(&value);
    }
    else if abi_data == ABI_VALUE_U16 + ABI_ARRAY_BIT {
        let mut value: [u16; 5] = [0;5];
        read_result_comap_fixed_array_u16(".namespace.keyname", &mut value).unwrap();
        push_costack_array_u16(&value);
    }
    else if abi_data == ABI_VALUE_U32 + ABI_ARRAY_BIT {
        let mut value: [u32; 5] = [0;5];
        read_result_comap_fixed_array_u32(".namespace.keyname", &mut value).unwrap();
        push_costack_array_u32(&value);
    }
    else if abi_data == ABI_VALUE_U64 + ABI_ARRAY_BIT {
        let mut value: [u64; 5] = [0;5];
        read_result_comap_fixed_array_u64(".namespace.keyname", &mut value).unwrap();
        push_costack_array_u64(&value);
    }
    else if abi_data == ABI_VALUE_I8 + ABI_ARRAY_BIT {
        let mut value: [i8; 5] = [0;5];
        read_result_comap_fixed_array_i8(".namespace.keyname", &mut value).unwrap();
        push_costack_array_i8(&value);
    }
    else if abi_data == ABI_VALUE_I16 + ABI_ARRAY_BIT {
        let mut value: [i16; 5] = [0;5];
        read_result_comap_fixed_array_i16(".namespace.keyname", &mut value).unwrap();
        push_costack_array_i16(&value);
    }
    else if abi_data == ABI_VALUE_I32 + ABI_ARRAY_BIT {
        let mut value: [i32; 5] = [0;5];
        read_result_comap_fixed_array_i32(".namespace.keyname", &mut value).unwrap();
        push_costack_array_i32(&value);
    }
    else if abi_data == ABI_VALUE_I64 + ABI_ARRAY_BIT {
        let mut value: [i64; 5] = [0;5];
        read_result_comap_fixed_array_i64(".namespace.keyname", &mut value).unwrap();
        push_costack_array_i64(&value);
    }
    else {
